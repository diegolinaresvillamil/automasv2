...
@Component({
  ...
  styleUrl: './ciudad.scss',
})
export class Ciudad {
  private route = inject(ActivatedRoute);

  /**
   * ✅ Slug dinámico de la URL (ej: /ciudad/bogota)
   * Si no existe, cae a 'automas'
   */
  readonly ciudadSlug$ = this.route.paramMap.pipe(
    map((p) => (p.get('slug') || 'automas').toLowerCase())
  );

  /**
   * ✅ Nombre de ciudad “bonito” para usar en el template
   * (si luego quieres mapearlo contra una tabla CIUDADES, me dices y lo dejamos full)
   */
  readonly ciudadNombre$ = this.ciudadSlug$.pipe(
    map((slug) => this.prettyCityName(slug))
  );

  private prettyCityName(slug: string): string {
    const cleaned = (slug || '').replace(/-/g, ' ').trim();
    if (!cleaned) return 'AutoMás';
    return cleaned
      .split(' ')
